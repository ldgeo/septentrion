...
def get_applied_versions():
    ...
    return utils.sortversions(applied_versions & known_versions)
...
def get_schema_version():
    ...
    version = get_closest_version(
        ...
        sql_tpl=settings.SCHEMA_TPL,
        existing_files=files.get_known_schemas(),
        force_version=settings.SCHEMA_VERSION)

    if version is None:
        raise ValueError('Can not find a schema to init the DB.')
    return version
...
def get_fixtures_version(target_version):
    ...
    version = get_closest_version(
        ...
        sql_tpl=settings.FIXTURES_TPL)

    if version is None:
        raise ValueError('Can not find fixtures to init the DB.')
    return version
